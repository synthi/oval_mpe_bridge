...
function oval_ui.draw(state)
  ...
  if state.ui_page == 1 then
    screen.move(0, 10) screen.text(oval_scales.names[params:get("oval_scale")])
    screen.move(128, 10) screen.text_right("TR: " .. params:get("oval_transpose"))
    local cx, cy, r = 64, 38, 20
    for i=1, 8 do
      local note_str = musicutil.note_num_to_name(state.active_notes[i], true)
      screen.level(state.pad_status[i].pressed and 15 or 4)
      if i == 1 then
        screen.move(cx, cy + 3) screen.text_center(note_str)
        ...
        screen.move(nx, ny + 3) screen.text_center(note_str)
        if state.pad_status[i].pressed then screen.circle(nx, ny, 6) screen.stroke() end
      end
    end
  elseif state.ui_page == 2 then
    screen.move(0, 10) screen.text("EDIT CUSTOM " .. state.ui_edit_slot)
    screen.move(0, 30) screen.level(5) screen.text("Pad " .. state.ui_edit_pad .. ": ")
    screen.level(15)
    ...
    screen.move(0, 50) screen.level(3) screen.text("E1:Slot E2:Pad E3:Note K3:Save")
  ...
  elseif state.ui_page == 6 then
    screen.move(0, 10) screen.text("TOUCH SWITCH (CC2)")
    screen.move(0, 30) screen.text("Mode: " .. params:string("oval_cc2_mode"))
    if params:get("oval_cc2_mode") == 2 then
      screen.move(0, 40) screen.text("Dest CC: " .. params:get("oval_cc2_dest"))
    elseif params:get("oval_cc2_mode") == 3 then
      screen.move(0, 40) screen.text("Note: " .. params:get("oval_cc2_note"))
      screen.move(0, 50) screen.text("Velocity: " .. params:get("oval_cc2_vel"))
    end
  end
  
  for i=1, 6 do
    screen.level(state.ui_page == i and 15 or 2)
    screen.move(106 + (i*3), 60)
    screen.line_rel(2, 0)
    screen.stroke()
  end
end
